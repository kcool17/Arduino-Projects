...
public class RollingDie extends Die
{
  ...
  /**
   * Draws a given number of dots on this die
   * The parameter g refers to the Graphics object for drawing
   * The parameters x,y specify the location
   * The parameter numDots specifies the number of dots that need to be drawn (1-6)
   * Post: dot are drawn on the dice
   */
  private void drawDots(Graphics g, int x, int y, int numDots)
  {
    ...
    // Use the method: g.fillOval(x2, y2, dotSize, dotSize);

      //<Add code here>
    if (numDots == 1) {//Rolling 1
    	g.fillOval(x2, y2, dotSize, dotSize);
    }else if (numDots == 2) {//Rolling 2
    	g.fillOval(x1, y1, dotSize, dotSize);
    	g.fillOval(x3, y3, dotSize, dotSize);
    }else if (numDots == 3) {//Rolling 3
    	g.fillOval(x1, y1, dotSize, dotSize);
    	g.fillOval(x3, y3, dotSize, dotSize);
    	g.fillOval(x2, y2, dotSize, dotSize);
    }else if (numDots == 4) {//Rolling 4
    	g.fillOval(x1, y1, dotSize, dotSize);
    	g.fillOval(x3, y3, dotSize, dotSize);
    	g.fillOval(x3, y1, dotSize, dotSize);
    	g.fillOval(x1, y3, dotSize, dotSize);
    }else if (numDots == 5) {//Rolling 5
    	g.fillOval(x1, y1, dotSize, dotSize);
    	g.fillOval(x3, y3, dotSize, dotSize);
    	g.fillOval(x3, y1, dotSize, dotSize);
    	g.fillOval(x1, y3, dotSize, dotSize);
    	g.fillOval(x2, y2, dotSize, dotSize);
    }else if (numDots == 6) {//Rolling 6
    	g.fillOval(x1, y1, dotSize, dotSize);
    	g.fillOval(x3, y3, dotSize, dotSize);
    	g.fillOval(x3, y1, dotSize, dotSize);
    	g.fillOval(x1, y3, dotSize, dotSize);
    ...
    }else {//If a different number is given, give an error
    	System.out.println("Error, you suck at coding. And typing.");
    }
  }
}
