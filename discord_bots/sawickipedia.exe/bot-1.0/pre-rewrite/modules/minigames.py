...
class Minigames():
    ...
    async def blackjack(self, ctx, arg = "noArg"):
        ...
        if arg == "about":
            ...
            await self.bot.say(embed=bjEmbed)
            return
        if not playingBJ:    
            myBet = 0
            if arg == 'all':
            ...
            try:
                ...
                await self.bot.say("Please use the format `?bj bet`.")
                return
            if myBet > currentMoney:
                await self.bot.say("You don't even have that much on hand, you poor peasant. Try again when you have money.")
                return
            if myBet <=0:
                await self.bot.say("That's not enough money to bet.")
                return
            betBJ = myBet
            currentMoney-=betBJ
            playerCards = []
            ...
            if arg == "hit" or arg == "h":
                ...
                await self.bot.say("Game in Progress! Finish it first. If you forget what the game was, here:")
        if not doCalculate:
            for card in dealerCards:
                try:
                    card = int(card)
    ...
    @commands.command(pass_context=True, aliases=["rou"])
    async def roulette(self, ctx, bet = "noArg", place = "noArg"):
        ...
        if bet == "about":
            ...
            await self.bot.say(embed=rouEmbed)
            ...
            try:
                ...
                await self.bot.say("Invalid input! Use `?roulette bet place`.")

            if bet<=0:
                await self.bot.say("That's not a bet. Bet greater than 0.")
                return
            

            if bet>currentMoney:
                await self.bot.say("You're too poor for that bet. Try again.")
                return
            ...
            if playingRoulette == True:
                await self.bot.say("You're in the middle of a game of one now. Just wait.")
                return
            ...
            if isInt:
                if int(place) >0 and int(place) <=36:
                    ...
                    await self.bot.say("Invalid input! Make sure `place` is either a number from 1-36, 1st/2nd/3rd, red/black, or even/odd.")
                    return
            ...
            if rouletteOn == False:
                await self.bot.say("New roulette game started! Make your bets! You have 30 seconds.")
            else:
    ...
    @commands.command(pass_context=True, aliases=["triv"])
    async def trivia(self, ctx, arg = "noArg", *user : str):
        ...
        if arg == "about":
            ...
            await self.bot.say(embed=aboutTrivEmbed)
        elif arg == "q" or arg=="question":
            questionTrivEmbed = discord.Embed(title=str(self.bot.get_server(server).get_member(member)), description="You have 10 seconds.", color=0x00ff33)
            
            if questionCanGuess:
                await self.bot.say("Guess the previous one first!")
            else:
                ...
                rawCorrect = self.html_parser.unescape(self.getInfo(rawData, '"correct_answer":', ',"incorrect_answers"'))
                ...
                #Say Embed
                await self.bot.say(embed=questionTrivEmbed)
                #Pickle Dump
            ...
            if questionCanGuess:
                ...
                guess = int(guess)
                if guess == correctLoc:
                    await self.bot.say("Correct! You got $75!")
                    currentMoney += 75
                    pickle.dump(currentMoney, open("servers" + os.sep + str(server) + os.sep + str(member) + os.sep + "money.p", "wb"))
                    numCorrect += 1
                    numGuessed += 1
                    ...
                    await self.bot.say("Wrong! The right answer was: ***" + correctLetter + "***: *" + rawCorrect + "*")
                    numGuessed += 1
            ...
            if questionCanGuess:
                ...
                if guess == rawCorrect.lower():
                    await self.bot.say("Correct! You got $50!")
                    currentMoney += 50
                    pickle.dump(currentMoney, open("servers" + os.sep + str(server) + os.sep + str(member) + os.sep + "money.p", "wb"))
                    numCorrect += 1
                    numGuessed += 1
                ...
                await self.bot.say("Hey, hold on! Ask a question first! (Or if you did, wait for *all* answers to appear).")
        elif arg == "score":
            arg2 = ""
            for item in user:
                arg2 = arg2 + item + " "
            ...
            if self.bot.get_server(server).get_member(member) is None:
                ...
                try:
                    ...
                    numGuessed = 0
                scoreTrivEmbed = discord.Embed(title=str(self.bot.get_server(server).get_member(member)), color=0x000099)
                ...
                await self.bot.say(embed=scoreTrivEmbed)
        else:
            await self.bot.say("Sorry! That's not a valid trivia command! Try `?trivia about` for more info!")
