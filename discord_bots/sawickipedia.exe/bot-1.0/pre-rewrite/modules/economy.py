...
class Economy():
    ...
    @commands.command(pass_context=True, aliases=["bal", "mon"], description = 'Displays the amount of money either you have, or the person you inputted. Displays On Hand money, Bank, and Total money.')
    async def money(self, ctx, *user : str):
        ...
        if self.bot.get_server(server).get_member(member) is None:
            ...
            if len(findIDList)==1:
                ...
                await self.bot.say("No users found. Type it correctly next time.")
                return
            else:
        ...
        if self.bot.get_server(server).get_member(member) is None:
            await self.bot.say("Not a valid user, please try again.")
        else:
            try:
                currentMoney = pickle.load(open("servers" + os.sep + str(server) + os.sep + str(member) + os.sep + "money.p", "rb"))
            except:
            ...
            try:
                ...
                currentBank = 0
            balanceEmbed = discord.Embed(title=str(self.bot.get_server(server).get_member(member)) + "'s Money", color=0x00FF00)
            ...
            await self.bot.say(embed=balanceEmbed)

    @commands.command(pass_context=True, aliases=["money-adjust", "mon-adj", "money-adj", "moneyadj"])
        ...
        if member in ADMINS:
            ...
            if self.bot.get_server(server).get_member(member) is None:
                ...
                if len(findIDList)==1:
                    ...
                    return
                member = theID
            if self.bot.get_server(server).get_member(member) is None or amount =="":
                await self.bot.say("Invalid input, use `?money-adjust money @user`.")
            else:
                try:
                    currentMoney = pickle.load(open("servers" + os.sep + str(server) + os.sep + str(member) + os.sep + "money.p", "rb"))
                except:
                ...
                try:
                    ...
                    if int(amount)>0:
                        giveEmbed = discord.Embed(title="$" + str("{:,}".format(int(amount))) + " given to " + str(self.bot.get_server(server).get_member(member).display_name) + " by " + str(ctx.message.author.display_name) + ".", color=0x00FF00)
                    elif int(amount)<0:
                        giveEmbed = discord.Embed(title="$" + str("{:,}".format(abs(int(amount)))) + " taken from " + str(self.bot.get_server(server).get_member(member).display_name) + " by " + str(ctx.message.author.display_name) + ".", color=0x00FF00)
                    else:
                        giveEmbed = discord.Embed(title="No money given.", color=0x00FF00)
                    await self.bot.say(embed=giveEmbed)
                except:
                    await self.bot.say('Error, make sure "money" is an integer in `?money-adjust money @user`.')
                
        else:
            await self.bot.send_file(ctx.message.channel, 'nopower.gif')

    @commands.command(pass_context=True, aliases=["bank-adjust", "bank-adj", "bankadj"])
        ...
        if member in ADMINS:
            ...
            if self.bot.get_server(server).get_member(member) is None:
                ...
                if len(findIDList)==1:
                    ...
                    return
                member = theID
            if self.bot.get_server(server).get_member(member) is None or arg2 =="noArg":
                await self.bot.say("Invalid input, use `?bank-adjust money @user`.")
            else:
                try:
                ...
                try:
                    ...
                    if int(arg2)>0:
                        giveEmbed = discord.Embed(title="$" + "{:,}".format(int(arg2)) + " given to " + str(self.bot.get_server(server).get_member(member).display_name) + "'s bank by " + str(ctx.message.author.display_name) + ".", color=0x00FF00)
                    elif int(arg2)<0:
                        giveEmbed = discord.Embed(title="$" + str("{:,}".format(abs(int(arg2)))) + " taken from " + str(self.bot.get_server(server).get_member(member).display_name) + " by " + str(ctx.message.author.display_name) + ".", color=0x00FF00)
                    else:
                        giveEmbed = discord.Embed(title="No money given.", color=0x00FF00)
                    await self.bot.say(embed=giveEmbed)
                except:
                    await self.bot.say('Error, make sure "money" is an integer in `?bank-adjust money @user`.')
                
        else:
            await self.bot.send_file(ctx.message.channel, 'nopower.gif')


    @commands.command(pass_context=True, aliases=["dep"], description = 'Deposits the money you have. You can deposit a specific amount, or all on hand. Format: "?dep all", or "?dep amount".')
    async def deposit(self, ctx, amount : str):
        ...
        if arg =="all":
            if currentMoney <= 0:
                await self.bot.say("You have nothing to deposit.")
            else:
                currentBank += currentMoney
                depEmbed = discord.Embed(title="$" + "{:,}".format(currentMoney) + " deposited!", color = 0x00FF00)
                await self.bot.say(embed=depEmbed)
                currentMoney = 0
        else:
            try:
                if int(arg) > currentMoney:
                    await self.bot.say("You're not that rich. Deposit what you can.")
                elif int(arg) < 0:
                    await self.bot.say("Use `?with` to withdraw money.")
                else:
                    currentBank += int(arg)
               ...
               await self.bot.say("Invalid input, use `?dep amount`.")
        pickle.dump(currentMoney, open("servers" + os.sep + str(server) + os.sep + str(member) + os.sep + "money.p", "wb"))
    ...
    async def withdraw(self, ctx, amount = "noArg"):
        ...
        if arg =="all":
            if currentBank <= 0:
                await self.bot.say("You have nothing in your bank. Sad, really.")
            else:
                currentMoney += currentBank
                withEmbed = discord.Embed(title="$" + "{:,}".format(currentBank) + " withdrawn!", color = 0x00FF00)
                await self.bot.say(embed=withEmbed)
                currentBank = 0
        else:
            try:
                if int(arg) > currentBank:
                    await self.bot.say("You're not that rich. Withdraw what you have.")
                elif int(arg) < 0:
                    await self.bot.say("Use `?dep` to deposit money.")
                else:
                    currentBank -= int(arg)
                    currentMoney += int(arg)
               ...
               await self.bot.say("Invalid input, use `?with amount`.")
        pickle.dump(currentMoney, open("servers" + os.sep + str(server) + os.sep + str(member) + os.sep + "money.p", "wb"))
        pickle.dump(currentBank, open("servers" + os.sep + str(server) + os.sep + str(member) + os.sep + "bank.p", "wb"))


    @commands.command(pass_context=True, description = 'Gives money to someone of your choice.')
    async def give(self, ctx, amount : str, *person : str):
        ...
        if self.bot.get_server(server).get_member(member) is None:
            ...
            if len(findIDList)==1:
                ...
                return
            member = theID
        if self.bot.get_server(server).get_member(member) is None or arg2 == "":
            await self.bot.say("Invalid input, use `?give money @user`.")
        else:
            try:
            ...
            try:
                if int(arg2)<=0:
                    await self.bot.say("Don't try and exploit this anymore. My master fixed it. Use a positive number.")
                    return
                if int(arg2)>giverMoney:
                    ...
                    if int(arg2)>0:
                        giveEmbed = discord.Embed(title="$" + "{:,}".format(int(arg2)) + " given to " + str(self.bot.get_server(server).get_member(member).display_name) + " by " + str(ctx.message.author.display_name) + ". How kind.", color=0x00FF00)
                    elif int(arg2)<0:
                ...
                await self.bot.say('Error, make sure "money" is an integer in `?give money @user`.')


    @commands.command(pass_context=True, aliases=["lb", "top", "highscore"], description = 'Displays the server\'s leaderboard. Use "?lb page" for a different page.')
    async def leaderboard(self, ctx, page="1"):
        ...
        lbEmbed = discord.Embed(title=str(self.bot.get_server(server))+ " Leaderboard", description="---------------------------------------------", color=0xFF0000)
        x = begIn
        y = 0
        while x <= endIn:
            try:
        ...
        await self.bot.say(embed=lbEmbed)

    @commands.command(pass_context=True, aliases=["reset-economy", "reset-eco", "reseteco"], description = 'Admin-only. Resets all money in the server. Make sure you want to do this for real.')
            ...
            if arg == "I_REALLY" and arg2 == "wantToDoThis":
                for member in ctx.message.server.members:
                    ...
                    try:
                        ...
                        pass
                await self.bot.say("Done! I am yours to command! *...mostly.*")
            else:
                await self.bot.say("You sure? This resets *all* money in this server. If you are sure, use `?reset-economy I_REALLY wantToDoThis`.")
        else:
            await self.bot.send_file(ctx.message.channel, 'nopower.gif')

    @commands.command(pass_context=True, aliases=["give-to-all"],  description = 'Admin-only. Gives a certain amount of money to everyone in the server.')
        ...
        if ctx.message.author.id in ADMINS:
            for member in ctx.message.server.members:
                ...
                try:
                    ...
                    await self.bot.say("Please use a valid sum of money.")
                    errorThing = True
                    break
                ...
                try:
                    ...
                    pass
            if not errorThing:
                await self.bot.say("Done! Enjoy the money, everyone!")
        else:
            await self.bot.send_file(ctx.message.channel, 'nopower.gif')


    @commands.command(pass_context=True, aliases=["set-salary", "set-sal", "setsal"])
    async def setsalary(self, ctx, role : str, salary : str):
        ...
        if member in ADMINS:
            ...
            if not os.path.exists("servers" + os.sep + str(server) + os.sep + str(role)) or arg2 == "":
                await self.bot.say("Invalid input, use `?set-salary @role salary`.")
            else:
                try:
                ...
                try:
                    ...
                    await self.bot.say('Error, make sure "salary" is an integer in `?set-salary @role salary`.')
    ...
    @commands.command(pass_context=True, aliases=["give-salary", "give-sal", "givesal"], description='Admin-only. Distributes salary, even if it\'s not the right time.')
    async def givesalary(self, ctx):
        ...
        if ctx.message.author.id in ADMINS:
            ...
            for role in self.bot.get_server(server).roles:
                try:
    ...
    @commands.command(pass_context=True, aliases=["sal"], description = 'Used to check the salaries of each role in the server. If it\'s $0, it won\'t show.')
    async def salary(self, ctx):
        ...
        await self.bot.say(embed=salEmbed)



    @commands.command(pass_context=True, aliases=["set-channel", "setchan", "set-chan"], description = 'Admin-only. Sets the channel for bot notifications. Uses the default one by default. Format: "?setchannel #channel".')
        ...
        if ctx.message.author.id in ADMINS:
            for thing in ctx.message.server.channels:
                if ctx.message.server.get_channel(arg[2:len(arg)-1]) == thing:
                    ...
                    setChannel=True
            if not setChannel:
                await self.bot.say("Error, invalid channel.")
        else:
            await self.bot.send_file(ctx.message.channel, 'nopower.gif')

    @commands.command(pass_context=True, aliases=["reset-salary", "reset-sal", "resetsal"])
    async def resetsalary(self, ctx, arg="I am ", arg2="unsure about this."):
        ...
        if ctx.message.author.id in ADMINS:
            if arg == "I_REALLY" and arg2 == "wantToDoThis":
                ...
                await self.bot.say("You sure? This resets *all* wages in this server. If you are sure, use `?reset-salary I_REALLY wantToDoThis`.")
        else:
            await self.bot.send_file(ctx.message.channel, 'nopower.gif')

    @commands.command(pass_context=True, aliases=["job"], description = 'Work for a small amount of money. Always succeeds, but only can be done sometimes.')
    async def work(self, ctx):
        ...
        if workCooldown < 0:
            ...
            pickle.dump(currentMoney, open("servers" + os.sep + str(server) + os.sep + str(member) + os.sep + "money.p", "wb"))
            await self.bot.say(embed=workEmbed)
            workCooldown = coolWork
            pickle.dump(workCooldown, open("servers" + os.sep + str(server) + os.sep + str(member) + os.sep + "workCooldown.p", "wb"))
        else:
            await self.bot.say("Please wait " + "{:,}".format(workCooldown) + " seconds before working again!")

    @commands.command(pass_context=True, aliases=["set-work"], description = 'Admin-only. Set max, min, and cooldown values of ?work.')
    ...
    @commands.command(pass_context=True, description = 'Crime for a large amount of money. Sometimes fails, and you lose money instead.')
    async def crime(self, ctx):
        ...
        if crimeCooldown < 0:
            ...
            if didSucceed==1:
                ...
                currentMoney += randomMoney
                pickle.dump(currentMoney, open("servers" + os.sep + str(server) + os.sep + str(member) + os.sep + "money.p", "wb"))
                await self.bot.say(embed=crimeEmbed)
            else:
            ...
            await self.bot.say("Please wait " + "{:,}".format(crimeCooldown) + " seconds before being a criminal again!")

    @commands.command(pass_context=True, aliases=["set-crime"], description = 'Admin-only. Set max, min, winrate, and cooldown values of ?crime. Losing = you lose a number between min and max. Winning = gaining that number.')
    ...
    @commands.command(pass_context=True, aliases=["steal"], description = 'Rob money from a member\'s On Hand. It may fail, and you\'ll lose instead.')
    async def rob(self, ctx, *person : str):
        ...
        if self.bot.get_server(server).get_member(member) is None:
            ...
            member = theID
        if self.bot.get_server(server).get_member(member) is None:
            await self.bot.say("Invalid input, use `?rob @user`.")
        else:
            if member == ctx.message.author.id:
                await self.bot.say("Robbing yourself is a stupid idea. Stop it.")
                return
            try:
            ...
            if robCooldown < 0:
                ...
                if robbedMoney<=0:
                    robEmbed=discord.Embed(title=str(ctx.message.author) + "'s Robbery of " + str(self.bot.get_server(server).get_member(member)), description="They didn't even have enough money to rob! Idiot.", color=0xFF0000)
                    ...
                    await self.bot.say(embed=robEmbed)
                elif didSucceed==1:
                    robEmbed=discord.Embed(title=str(ctx.message.author) + "'s Robbery of " + str(self.bot.get_server(server).get_member(member)), description=randomRob, color=0x00FF00)
                    ...
                    robEmbed=discord.Embed(title=str(ctx.message.author) + "'s Robbery of " + str(self.bot.get_server(server).get_member(member)), description=randomRob, color=0xFF0000)
                    robEmbed.add_field(name="Fined:", value= "$" + "{:,}".format(randomMoney), inline=True)
                    robberMoney -= randomMoney
                    pickle.dump(robberMoney, open("servers" + os.sep + str(server) + os.sep + str(ctx.message.author.id) + os.sep + "money.p", "wb"))
                    await self.bot.say(embed=robEmbed)
                ...
                await self.bot.say("Please wait " + "{:,}".format(robCooldown) + " seconds before trying to rob again!")

    @commands.command(pass_context=True, aliases=["set-rob", "set-steal", "setsteal"], description = 'Admin-only. Set max, min, winrate, and cooldown values of rob. Losing = you lose a number between min and max. Winning = rob around 85% of the person\'s On Hand.')
    ...
    @commands.command(pass_context=True, aliases=["set-salary-time", "setsaltime", "set-sal-time"], description = 'Admin-only. Sets the salary giving time.')
        ...
        if ctx.message.author.id in ADMINS:
            try:
                ...
                if hour < 24 and hour >= 0 and minute <60 and minute >= 0:
                    ...
                    await self.bot.say("Please input a valid time! (Military time)")
            except:
                await self.bot.say("Invalid input! Please use the format `?set-salary-time hour minute`, using military time.")
        else:
            await self.bot.send_file(ctx.message.channel, 'nopower.gif')


    @commands.command(pass_context=True, aliases=["get-salary-time", "getsaltime", "get-sal-time", "saltime", "sal-time"])
    async def getsalarytime(self, ctx):
        ...
        await self.bot.say("The salaries will be given at " + str(salHour) + ":" + str(salMinute) + " " + thePM)
